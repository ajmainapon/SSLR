...
medical CT?* A **leave-one-out (LOO) ablation** says they do **not** — the recipe that works is the
*inverse* of the natural-image default.

<p align="center">
  <img src="figures_a3/fig3_loo_ablation.png" width="620"><br>
...
> **Headline.** The recommended backbone — **A3 = multi-block masking + a _narrow_ slice gap
...
- **The I-JEPA recipe does not transfer.** A *narrow* slice gap beats a wide gap by **45–124%**, and
  ...
  additive (the combined backbone `a5` *loses* to A3 by 5–16%). **Recommended: narrow gap +
  multi-block (A3).** The slice gap is the dominant axis.
- **A3 beats random init at every N** — linear probe ≈ +100–125%; fine-tune +13.0% → +8.5% → +2.8%
  at N = 20/50/100 (4 seeds), significant at N = 20 and N = 50.
- **SSL reduces fine-tune seed variance 1.3–2.2×.**
- **Transfer holds:** frozen A3 features beat random init on **AMOS-CT (+158–260%)** and even
  **AMOS-MR (+41–48%)** — having never seen an MR slice.
- **Honest ceiling:** A3 fine-tune recovers **67%** of a fully-supervised nnU-Net at a matched
...
|---|---|---|---|
| `v2` | multi-block | wide | I-JEPA-style baseline |
| **`A3`** | **multi-block** | **narrow** | **headline** |
| `A1` | random | wide | masking inversion |
| `a5` | random | narrow | additivity test |
...
### Leave-one-out ablation — linear probe (single seed)
...
<p align="center">
  <img src="figures_a3/fig1_data_efficiency_linprobe.png" width="560"><br>
  <sub>A3 (frozen) vs the I-JEPA-style v2 vs random init; nnU-Net supervised reference dashed at 0.806.</sub>
</p>
...
scripts/
  make_figures_a3.py        result figures (--figs to regen a subset)
...
# Pretrain the A3 backbone (multi-block masking + narrow slice gap)
