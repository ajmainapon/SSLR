...
QUEUE_LOG="$ROOT/runs/queue_capacity_trainable_linear_n20.log"
...
echo "[$(date -Is)] trainable+linear N=20 capacity-control queue starting" | tee -a "$QUEUE_LOG"
...
for seed in 0 1 2 3; do
  ...
  for initialization in ssl random; do
    run_name="ftlin_a3_${initialization}_n20_seed${seed}"
...
echo "[$(date -Is)] ALL 8 N=20 TRAINABLE+LINEAR RUNS COMPLETE" | tee -a "$QUEUE_LOG"
