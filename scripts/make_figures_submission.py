"""Generate paper-ready figures using the submitted-paper naming convention.

Paper names map to historical run identifiers as follows:
    I-JEPA-CT -> v2
    A1        -> loo_a1_random_mask
    A2        -> loo_a3_narrow_k (selected configuration)
    A3        -> loo_a5_dense_narrow

Reads runs/*/log.json files and writes four figures used in paper_draft.tex:

    figures_submission/
      fig1_data_efficiency_linprobe.png   # A2 vs I-JEPA-CT vs Random
      fig2_per_organ_a2_ft_n50.png        # Per-organ A2 SSL fine-tune at N=50
      fig3_factorial_ablation.png         # I-JEPA-CT/A1/A2/A3 factorial comparison
      fig4_finetune_multiseed.png         # A2 SSL vs Random with 4-seed error bars
...
Usage:
    ...
    python scripts/make_figures_submission.py --runs_root runs --out figures_submission
"""
import argparse
import json
...
COL_A2      = "#2E86AB"   # blue: A2 (selected configuration)
COL_IJEPA   = "#A23B72"   # purple: I-JEPA-CT baseline
COL_A1      = "#F18F01"   # orange: A1 (random masking ablation)
COL_A3      = "#3FA34D"   # green: A3 (random masking + narrow gap)
...
def collect_a2_ft(runs_root, n):
    """4 seeds of paper A2 SSL fine-tune at N (legacy run prefix: ft_a3)."""
    vals = []
    for s in (0, 1, 2, 3):
...
def collect_amos_a2(runs_root, n):
    """4 seeds of AMOS paper A2 SSL linear probe (legacy run prefix: a3)."""
    vals = []
...
def fig1_data_efficiency(runs_root, out):
    Ns = [5, 10, 20, 50, 100]
    a2  = [best_val_fg(runs_root / f"lin_loo_a3_n{n}")  for n in Ns]
    v2  = [best_val_fg(runs_root / f"lin_v2_ssl_n{n}")  for n in Ns]
    rnd = [best_val_fg(runs_root / f"lin_v2_random_n{n}") for n in Ns]

    fig, ax = plt.subplots(figsize=(6.5, 4.5))
    ax.plot(Ns, a2,  "o-",  label="A2 SSL (multi-block mask + narrow k)",
            color=COL_A2,     linewidth=2.0, markersize=8)
    ax.plot(Ns, v2,  "s--", label="I-JEPA-CT SSL (baseline)",
            color=COL_IJEPA,  linewidth=1.5, markersize=7, alpha=0.85)
...
# ---------------------------------------------------------------------------
# Figure 2 — Per-organ Dice at paper A2 fine-tune N=50
# ---------------------------------------------------------------------------

def fig2_per_organ_ft(runs_root, out):
    # Matched budget: both A2 fine-tune and the nnU-Net reference use N=50.
    ...
    w = 0.4
    ax.bar(x - w/2, a3,  w, label="A2 SSL FT (N=50, seed=0)", color=COL_A2,     edgecolor="black", linewidth=0.5)
    ax.bar(x + w/2, nnu, w, label="nnU-Net supervised (N=50)", color=COL_RANDOM, edgecolor="black", linewidth=0.5, alpha=0.75)

    # Annotate recovery % above each A2 bar
    for i, (v, n) in enumerate(zip(a3, nnu)):
        pct = 100.0 * v / n
        ax.annotate(f"{pct:.0f}%", xy=(x[i] - w/2, v + 0.015),
                    ha="center", fontsize=8, fontweight="bold", color=COL_A2)

    ax.set_xticks(x)
    ax.set_xticklabels(ORGANS, rotation=30, ha="right")
    ax.set_ylabel("Foreground Dice")
    ax.set_title(f"Per-organ Dice at A2 SSL fine-tune, N=50 (mean fg = {a3.mean():.4f})")
    ax.legend(loc="upper right", framealpha=0.95)
    ax.grid(True, alpha=0.3, axis="y")
    ax.set_ylim(0, 1.0)
    fig.tight_layout()
    fig.savefig(out / "fig2_per_organ_a2_ft_n50.png", dpi=150)
    fig.savefig(out / "fig2_per_organ_a2_ft_n50.pdf")
    plt.close(fig)
    print(f"  saved {out}/fig2_per_organ_a2_ft_n50.{{png,pdf}}")


# ---------------------------------------------------------------------------
# Figure 3 — submitted-paper factorial naming under linear probing
# ---------------------------------------------------------------------------

def fig3_factorial_ablation(runs_root, out):
    Ns = [5, 10, 20, 50, 100]
    v2 = [best_val_fg(runs_root / f"lin_v2_ssl_n{n}")    for n in Ns]
    ...
    ax.plot(Ns, v2, "s--", label="I-JEPA-CT (multi-block mask + wide k)",
            color=COL_IJEPA, linewidth=1.5, markersize=7)
    ax.plot(Ns, a1, "^-",  label="A1 (random mask + wide k)",
            color=COL_A1, linewidth=1.5, markersize=7)
    if all(v is not None for v in a5):
        ax.plot(Ns, a5, "D--", label="A3 (random mask + narrow k)",
                color=COL_A3, linewidth=1.5, markersize=6)
    else:
        print("  [fig3] WARN: a5 runs not found — plotting without a5")
    ax.plot(Ns, a3, "o-",  label="A2 (multi-block mask + narrow k; selected)",
            color=COL_A2, linewidth=2.0, markersize=8)
    ...
    _readme_clean(ax)
    fig.tight_layout()
    fig.savefig(out / "fig3_factorial_ablation.png", dpi=160)
    fig.savefig(out / "fig3_factorial_ablation.pdf")
    plt.close(fig)
    print(f"  saved {out}/fig3_factorial_ablation.{{png,pdf}}")
...
def fig4_finetune_multiseed(runs_root, out):
    ...
    fig, ax = plt.subplots(figsize=(6.5, 4.5))
    ax.errorbar(Ns, a3_means,  yerr=a3_stds,  fmt="o-",  color=COL_A2,
                linewidth=2.0, markersize=8, capsize=6, capthick=1.5,
                label="A2 SSL ft (4-seed mean ± std)")
    ...
    ax.set_title("End-to-end fine-tune (lr$_{bb}$=1e-4): A2 SSL vs Random init")
    ...
    print(f"  {'N':<5} {'A2 SSL mean±std':<24} {'Random mean±std':<24} {'Δ rel':<10}")
...
def fig5_amos_cross_dataset(runs_root, out):
    """Paper A2 backbone transferred to AMOS22 (frozen weights, linear probe).
    ...
    for n in Ns:
        a3  = collect_amos_a2(runs_root, n)
    ...
    ax.errorbar(Ns, amos_a3_means, yerr=amos_a3_stds, fmt="o-", color=COL_A2, linewidth=2.2, markersize=9, capsize=5, capthick=1.5,
            label="AMOS A2 SSL (cross-dataset, 4-seed mean ± std)")
    ...
    ax.plot(Ns, ts_a3,    "o--", color=COL_A2,     linewidth=1.5, markersize=7,
            alpha=0.55, label="TotalSeg A2 SSL (in-dataset, for comparison)")
    ...
    ax.set_xlabel("Number of labeled training volumes (log scale)")
    ax.set_ylabel("Linear-probe mean foreground Dice")
    ax.set_title("Cross-dataset transfer: A2 backbone (TotalSeg-pretrained) on AMOS22\n"
    ...
    print(f"  {'N':<5} {'TS-A2':<8} {'TS-Rnd':<8} {'AMOS-A2 mean±std':<20} {'AMOS-Rnd mean±std':<20} {'TS gap':<10} {'AMOS gap':<10}")
...
def fig6_mri_cross_modality(runs_root, out):
    """CT-pretrained paper A2 backbone applied to AMOS-MR (frozen, linear probe).
    ...
    # AMOS-CT cross-dataset A2 for a dashed CT reference (shows modality erosion).
    amos_a3 = np.array([collect_amos_a2(runs_root, n).mean() for n in Ns])

    fig, ax = plt.subplots(figsize=(7.0, 4.8))
    ax.errorbar(Ns, a3_mean, yerr=a3_std, fmt="o-", color=COL_A2, linewidth=2.2,
                markersize=9, capsize=4,
                label=f"AMOS-MR A2 SSL (CT-pretrained, frozen, {n_seeds} seeds)")
    ...
    ax.plot(Ns, amos_a3, "o--", color=COL_A2, linewidth=1.5, markersize=7,
            alpha=0.5, label="AMOS-CT A2 SSL (cross-dataset, for comparison)")

    for n, a3m, rndm in zip(Ns, a3_mean, rnd_mean):
        rel = 100.0 * (a3m - rndm) / rndm
        alignment = "left" if n == Ns[0] else "center"
        ax.annotate(f"+{rel:.1f}%", xy=(n, a3m + 0.012), ha=alignment,
                    fontsize=10, fontweight="bold", color=COL_A2)
    ...
    ax.set_ylabel("Linear-probe mean foreground Dice")
    ax.set_title("Cross-modality transfer: CT-pretrained A2 backbone on AMOS-MR\n"
    ...
    print(f"  {'N':<5} {'MR-A2 mean±std':<18} {'MR-Rnd mean±std':<18} {'MR gap':<10}")
    for i, n in enumerate(Ns):
...
def main():
    ...
    p.add_argument("--out", default="figures_submission",
    ...
    if 2 in want:
        print("Figure 2: Per-organ Dice at A2 SSL ft N=50 (matched to nnU-Net N=50)")
        fig2_per_organ_ft(runs_root, out)

    if 3 in want:
        print("Figure 3: masking × slice-gap factorial comparison")
        fig3_factorial_ablation(runs_root, out)
    ...
    print("LaTeX inclusion: figures_submission/fig1_data_efficiency_linprobe.png")
